...
public class StudentManagementSystem {
    ...
    public static void main(String[] args) {
        try {
            ...
            String url = "jdbc:mysql://localhost/spec", user = "root", password = "root";
            ...
            sql = "SELECT S.name, S.roll_number, SP.need_wc, SP.is_vi " + "FROM student_data AS S "
            ...
            while (studentData.next()) {
                ...
                if (studentData.getString("need_wc") == null || studentData.getString("is_vi") == null) {
                    s[j++] = new Student(rno, nm);
                } else {
                    needsWC = studentData.getString("need_wc").charAt(0);
                    isVI = studentData.getString("is_vi").charAt(0);
                    s[j++] = new SpecialNeedsStudent(rno, nm,
                            (needsWC == '1' || needsWC == '1') ? true : (false),
                            (isVI == '1' || isVI == '1') ? true : (false));
                }
            }
            ...
            do {
                ...
                System.out.println("\t7. Print feedback");
                ...
                switch (choice) {
                    ...
                    case 2:
                        ...
                        for (int i = 0; i < numberOfStudents; i++) {
                            if (s[i].getRollNumber().equals(rno)) {
                                System.out.println(
                                        "The name of the student with roll number " + rno + " is " + s[i].getName());
                                flag = 'Y';
                            }

                        }
                        if (flag == 'N') {
                            System.out.println("The student with roll number " + rno + " doesn't exist.");
                        }
                        break;
                    case 3:
                        System.out.print("Enter roll number: ");
                        ...
                        for (int i = 0; i < numberOfStudents; i++) {
                            if (s[i].getRollNumber().equals(rno)) {
                                System.out.print("Enter a new name for the student: ");
                                nm = sc.next();
                                s[i].setName(nm);
                    ...
                    case 4:
                        ...
                        for (int i = 0; i < numberOfStudents; i++) {
                            if (s[i].getRollNumber().equals(rno)) {
                                s[i].setName("--removed--");
                                sql = "DELETE FROM special_needs WHERE student_id IN (SELECT id FROM student_data WHERE roll_number = '"
                                        + rno + "' )";
                    ...
                    case 5:
                        ...
                        break;
                    case 0:
                    	 System.out.println("Thank you for using the Student Management System. Please provide your feedback:");
                         sc.nextLine(); 
                         String feedback = sc.nextLine();
                         //System.out.println(feedback);
                         sql = "INSERT INTO feedback (feedback_text) VALUES ('" + feedback + "')";
                         res = stmt.executeUpdate(sql);
                         // Save the feedback in the database or perform any other desired action
                         System.out.println("Thank you for your feedback!");
                         break;
                      // printing the feedback      
                    case 7:
                      // Retrieving and printing saved feedback
                         //System.out.println("Saved Feedback:");
                         sc.nextLine();
                         System.out.println("Recent feedback");
                         sql = "SELECT * FROM feedback";
                         ResultSet feedbackData = stmt.executeQuery(sql);
                         password = sc.next();
                         if(password.equals("123456")) {
                        	 while (feedbackData.next()) {
                                 int feedbackId = feedbackData.getInt("id");
                                 String feedbackText = feedbackData.getString("feedback_text");
                                 Timestamp timestamp = feedbackData.getTimestamp("timestamp");
                                 
                                 System.out.println("Feedback ID: " + feedbackId);
                                 System.out.println("Feedback Text: " + feedbackText);
                                 System.out.println("Timestamp: " + timestamp);
                                 System.out.println("These are the recent feedback");
                             }
                         }
                         else {
...
// end text
                         break;
                     default:
                         System.out.println("ERROR! Enter a value between 1 and 7.");
                         break;
                }
